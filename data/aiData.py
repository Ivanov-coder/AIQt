...
def start(*choice: str) -> tuple[str, str]:
    ...
    SPARK_LINK = api["spark_conf"]["Spark_Link"]
    SPARK_MODEL_KEYS = api["spark_conf"]["model_key"]
    OTHER_LINK = api["other_conf"]["Link"]
    OTHER_KEY = api["other_conf"]["Key"]
    try:
        match choice:
            case ["1", str(model)]:  # Invoke Spark API
                utils.settings.logger.info(msg=f"Invoking Spark {model.upper()} API...")
                specific_model_key = f"{model}_Key"
                spark = _Spark(
                    model=model,
                    api=api,
                    link=SPARK_LINK,
                    apiKey=SPARK_MODEL_KEYS[specific_model_key],
                )
