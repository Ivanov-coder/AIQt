...
def switch(choice: str) -> None:
    ...
    if choice == "web-spark":
        ...
        return spark.CallSparkAI(
            model="lite").callByhttpx(random_id=randID)
    elif choice == "web-other":
        from webAI import other
        return other.CallOtherAI(
            model="qwen-long").callByhttpx(random_id=randID)

    elif choice == "local":
        import localAI
        # TODO: 这里的实例化需要做成选择框给用户选择模型
        return localAI.ollamallm.CallOllamaAI(
            model="llama3.1").callByOllama(random_id=randID)
